...
public class JDBCTable<T extends JDBCRecord> {
	...
	/*public JDBCTable<T> createIndex(OrderBy o) throws SQLException {
		return createIndex(o);
	}*/
	/*private String indexName(String[] columnNames) {
		return name()+"_"+Util.join("__",columnNames).replaceAll("\\s", "").replaceAll("\\W", "_");
	}*/
	public JDBCTable<T> createIndex(OrderBy o) throws SQLException {
    ...
    public JDBCRecordCursor<T> lookup(String columnSpec, Object... objects) throws SQLException {
        return lookup(OrderBy.parse(columnSpec), objects);
    }
	public JDBCRecordCursor<T> lookup(OrderBy ord, Object... objects) throws SQLException {
	    StringBuilder buf=new StringBuilder();
	    String cmd="";
		...
		return new JDBCRecordCursor<T>(rec, db.execQuery(selectFrom()+" where "+buf+" order by "+ord+";",objects) );
	}
	public JDBCRecordCursor<T> scope(String columnSpec,  Object[] from, Object[] to) throws SQLException {
	    return scope(OrderBy.parse(columnSpec),from,to);
	}
	public JDBCRecordCursor<T> scope(OrderBy ord, Object[] from, Object[] to) throws SQLException {
        StringBuilder buf=new StringBuilder();
        String cmd="";
        ...
        return new JDBCRecordCursor<T>(rec, db.execQuery(selectFrom()+" where "+buf+" order by "+ord+";",values));
	}
	public JDBCRecordCursor<T> scope(String singleColumnSpec, Object from, Object to) throws SQLException {
		return scope( OrderBy.parse(singleColumnSpec), JDBCRecord.q(from), JDBCRecord.q(to));
	}
	public int insertValues(Object... values) throws SQLException {
		return db.execUpdate("insert into "+nameSym()+"("+columnNameList(columns.toArray(new Column[0]))+") values ("+questions(values.length)+");", values);
	}
	public int insert(JDBCRecord r) throws SQLException {
		return insertValues(r.toValues());
	}
	public int updateValues(Object primaryKeyValue, Object... values) throws SQLException {
		Object[] values_prim=new Object[values.length+1];
		for (int i=0 ; i<values.length ; i++) {
			values_prim[i]=values[i];
		}
		values_prim[values_prim.length-1]=primaryKeyValue;
		return db.execUpdate("update "+nameSym()+" set "+set()+" where "+symbol(primaryKeyName())+"=?", values_prim);
	}
	...
	public void update(JDBCRecord r) throws SQLException {
		try {
			updateValues(r.getField(r.primaryKeyName()).get(this)  , r.toValues(true));
