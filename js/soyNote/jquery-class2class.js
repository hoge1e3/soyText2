 ...
 *      which are elements contained in <tag class=cssName> ... </tag>.
 *      subparts should have CSS-class names staring with "cssName-", following their own names.
 ...
 *           The Javascript-class will have accessor bar() and baz(), 
 *           which gets the subparts having CSS-class of "foo-bar" and "foo-baz" respectively.
             ex.   <span id="f1" class="foo">    <!-- to get this parts f=Class2class.get($("#f1")) -->
			          <p class="foo-bar">Bar</p> <!-- subparts. can be get by f.bar(); -->
			          <p class="foo-baz">Baz</p> <!-- subparts. can be get by f.baz(); -->
				   </span>
 ...
 *       YYYY is subPart's name. XXXX is an event type (upper or lower cases are accepted)
 *                     
 * @param {String} cssName   the CSS-class name to which bounded to this class. 
           This argument is optional, if this class have the method "cssName", which returns the CSS-class name.
 * @param {String} subParts  the comma-separated names which indicates subParts' names.
...
Function.prototype.bindCSS=function (cssName, subParts) {
   ...
   if (!cssName && klass.prototype.cssName) { cssName=klass.prototype.cssName(); }
   if (!cssName) { throw "cssName is null!"; }
   if (!klass.prototype.cssName) { klass.prototype.cssName=function (){return cssName;}; }
   if (!subParts && klass.prototype.subPartNames) {subParts=klass.prototype.subPartNames();}
   if (!subParts && klass.prototype.subPartsNames) {subParts=klass.prototype.subPartsNames();}
